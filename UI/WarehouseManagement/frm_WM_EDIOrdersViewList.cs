...
{
    public partial class frm_WM_EDIOrdersViewList : frmBaseForm
    {
        private static readonly log4net.ILog log = log4net.LogManager.GetLogger(typeof(frm_WM_EDIOrdersViewList));
        ...
        public frm_WM_EDIOrdersViewList()
        {
            InitializeComponent();
        ...
        public frm_WM_EDIOrdersViewList(DateTime orderDate, int customerID, string EDI_OrderID_List)
        {
            InitializeComponent();
            this.dateEditFromDate.EditValue = DateTime.Now.AddDays(-2).ToShortDateString();
            this.dateEditToDate.EditValue = DateTime.Now.ToShortDateString();
        ...
        private void grvOrderDetai_LoadFormatRules()
        {
            ...
            //gridFormatRule5.Name = "FormatRuleQuantity";
            //formatConditionRuleExpression3.Expression = "[Quantity] > [StockQty]";
            //formatConditionRuleExpression3.PredefinedName = "Red Bold Text";
            //gridFormatRule5.Rule = formatConditionRuleExpression3;
            //gridFormatRule6.Name = "FormatRuleProduct";
            //formatConditionRuleValue3.Expression = "[ProductID] = 0";
            //formatConditionRuleValue3.PredefinedName = "Red Fill";
            //gridFormatRule6.Rule = formatConditionRuleValue3;
            //this.grvEDIOrderDetailsView.FormatRules.Add(gridFormatRule5);
            //this.grvEDIOrderDetailsView.FormatRules.Add(gridFormatRule6);
        }

        private void grvEDIOrderViewList_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
        }
        ...
        private void btn_ProcessSelected_Click(object sender, EventArgs e)
        {
            ...
            foreach (var index in this.grvEDIOrderViewList.GetSelectedRows())
            {
                try
                {
                    ...
                    {
                        XtraMessageBox.Show("Error ! Can not process ! " + ex.Message + "  !", "WMS", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    ...
                    {
                        XtraMessageBox.Show("Order date is later than today , Can not process !!!", "WMS", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    {
                        XtraMessageBox.Show("Error ! EDI Order ID is null !", "WMS", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }

                    string orderType = Convert.ToString(this.grvEDIOrderViewList.GetRowCellDisplayText(index, "OrderType"));

                    //'Import Dispatching Orders
                    if (orderType.Equals("DO"))
                    {
                        int processType = Convert.ToInt32(radioGroup_DispatchType.EditValue);
                        if (processType.Equals(7))
                        {
                            //the shelf life 1/2, 2/3
                            DA.Warehouse.EDIOrdersDA v_DaTheLongLife = new EDIOrdersDA();
                            int v_Ret = v_DaTheLongLife.STEDI_ProcessDispatchingOrder_Main(v_EDIOrderID, m_userName, 7);
                        }
                        else if (processType.Equals(11))
                        {
                            //the shelf life 1/2 + 14 days
                            DA.Warehouse.EDIOrdersDA v_DaTheLongLife = new EDIOrdersDA();
                            int v_Ret = v_DaTheLongLife.STEDI_ProcessDispatchingOrder_Main(v_EDIOrderID, m_userName, 11);
                        }
                        else if (processType.Equals(12))
                        {
                            //the shelf life > 6 months
                            DA.Warehouse.EDIOrdersDA v_DaTheLongLife = new EDIOrdersDA();
                            int v_Ret = v_DaTheLongLife.STEDI_ProcessDispatchingOrder_Main(v_EDIOrderID, m_userName, 12);
                        }
                        else if (processType.Equals(13))
                        {
                            //the shelf life > 60%
                            DA.Warehouse.EDIOrdersDA v_DaTheLongLife = new EDIOrdersDA();
                            int v_Ret = v_DaTheLongLife.STEDI_ProcessDispatchingOrder_Main(v_EDIOrderID, m_userName, 13);
                        }
                        else if (processType.Equals(2))
                        {
                            ...
                            DA.Warehouse.EDIOrdersDA v_DaTheLongLife = new EDIOrdersDA();
                            int v_Ret = v_DaTheLongLife.STEDI_ProcessDispatchingOrder_PickSequence(v_EDIOrderID, m_userName);
                        }
                        else
                        {
                            DA.Warehouse.EDIOrdersDA v_DaFun = new EDIOrdersDA();
                            int v_Ret = v_DaFun.STEDI_ProcessDispatchingOrder(v_EDIOrderID, m_userName);
                        }
                    }
                    //'Import Receiving Orders
                    else if (orderType.Equals("RO"))
                    {
                        DA.Warehouse.EDIOrdersDA v_DaFun = new EDIOrdersDA();
                        int v_Ret = v_DaFun.STEDI_ProcessReceivingOrder(v_EDIOrderID, m_userName);
                    }
                }
                catch (Exception ex)
                {
                    v_EDIOrderID = Convert.ToInt32(this.grvEDIOrderViewList.GetRowCellDisplayText(index, "EDI_OrderID"));
                    var dl = XtraMessageBox.Show("The order No. [" + v_EDIOrderID + "] is having error while processing.\nDo you want to keep continue / stop processing ?", "WMS", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
                    if (dl.Equals(DialogResult.Cancel)) break;
                }
            }
        ...
        private void btn_ProcessMainCustomer_Click(object sender, EventArgs e)
        {
            if (this.lke_Customer.EditValue == null) return;
            if (this.grvEDIOrderViewList.SelectedRowsCount <= 0)
            {
            ...
            foreach (var index in this.grvEDIOrderViewList.GetSelectedRows())
            {
                try
                {
                    if (Convert.ToInt16(this.grvEDIOrderViewList.GetRowCellValue(index, "ProcessingStatus")) == 2)
                    {
                        XtraMessageBox.Show("Error ! Can not process ! Orders are already processed !", "WMS", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
                    ...
                    try
                    {
                        ...
                        string currentDate = Convert.ToString(currentRow["OrderDate"]);
                        string date = Convert.ToString(this.grvEDIOrderViewList.GetRowCellValue(index, "OrderDate"));
                        v_OrderDate = Convert.ToDateTime(this.grvEDIOrderViewList.GetRowCellDisplayText(index, "OrderDate"));
                    }
                    catch (Exception ex)
                    {
                    ...
                    if (v_EDIOrderType.Equals("DO"))
                    {
                        ...
                        if (v_CustomerID.Equals(920) || v_CustomerID.Equals(919)) //' Chi moi ap dung cho VinMart
                        {
                            ...
                            if (v_ListCusClient.Count <= 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    int result = v_daEDI.STCustomerClientsInsert(v_CustomerID, v_CustomerClientCode, v_CustomerClientName, v_CustomerClientAddress, m_userName, v_newCustomerClientID);
                                    ...
                                    {
                                        XtraMessageBox.Show("Error ! Can not create New Client !", "WMS", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                    }
                        ...
                        if (processType.Equals(1))
                        {
                            ...
                            int v_Ret = v_DaStandard.STEDI_ProcessDispatchingOrder_Main(v_EDIOrderID, m_userName, 1);
                        }
                        else if (processType.Equals(2))
                        {
                            DA.Warehouse.EDIOrdersDA v_DaExp = new EDIOrdersDA();
                            int v_Ret = v_DaExp.STEDI_ProcessDispatchingOrder_Main_PickSequence(v_EDIOrderID, m_userName);
                        }
                        else if (processType.Equals(5))
                        {
                            DA.Warehouse.EDIOrdersDA v_DaTheLongLife = new EDIOrdersDA();
                            int v_Ret = v_DaTheLongLife.STEDI_ProcessDispatchingOrder_Main(v_EDIOrderID, m_userName, 5);
                        }
                        ...
                        {
                            //Dispatched based on the requirement from each Client
                            DA.Warehouse.EDIOrdersDA v_DaByClient = new EDIOrdersDA();
                            int DispatchingMethod = Convert.ToInt16(this.grvEDIOrderViewList.GetRowCellValue(index, "DispatchingMethodID"));
                            if (DispatchingMethod == 0)
                            {
                                return;
                            }
                            else
                            {
                                int v_Ret = v_DaByClient.STEDI_ProcessDispatchingOrder_Main(v_EDIOrderID, m_userName, DispatchingMethod);
                            }
                        }
                    ...
                    else if (v_EDIOrderType.Equals("RO"))
                    {
                        ...
                        //int v_Ret = v_DaRO.STEDI_ProcessReceivingOrder(v_EDIOrderID, m_userName);
                        DA.DataProcess<object> DP = new DataProcess<object>();
                        int v_Ret = DP.ExecuteNoQuery("STEDI_ProcessReceivingOrder " + v_EDIOrderID + ", '" + m_userName + "'");
                    }
                ...
                {
                    v_EDIOrderID = Convert.ToInt32(this.grvEDIOrderViewList.GetRowCellDisplayText(index, "EDI_OrderID"));
                    var dl = XtraMessageBox.Show(ex + " The order No. [" + v_EDIOrderID + "] is having error while processing.\nDo you want to keep continue / stop processing ?", "WMS", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
                    if (dl.Equals(DialogResult.Cancel)) break;
                }
            }
            this.btn_Refresh_Click(sender, e);
        ...
        private void dockPanelFindDuplicate_Expanded(object sender, DevExpress.XtraBars.Docking.DockPanelEventArgs e)
        {
        ...
        private void grvEDIOrderDetailsView_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
        ...
        private void grvEDIOrderDetailsView_CellValueChanging(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {


        }

        private void grvEDIOrderDetailsView_ValidatingEditor(object sender, DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs e)
        {
            //if (this.grvEDIOrderDetailsView.FocusedRowHandle < 0) return;
            //if (!this.ediDetailsSource[this.grvEDIOrderDetailsView.FocusedRowHandle].IsDiffEXP) e.Valid = false;

            //DateTime? currentExpDate = Convert.ToDateTime(e.Value);
            //if (currentExpDate != null)
            //{
            //    int countFind = this.stockSource.Select("UseByDate='" + currentExpDate + "'").Count();
            //    if (countFind <= 0)
            //    {
            //        e.Valid = false;
            //    }
        ...
        private void mmScanbarcode_KeyDown(object sender, KeyEventArgs e)
        {
            if (!e.KeyCode.Equals(Keys.Enter)) return;
            // Lookup customer number

            string scanValue = this.mmScanbarcode.Text.Trim();
            var dataSource = (DataTable)this.grdEDIOrderList.DataSource;
            var rowsLookup = dataSource.Select("CustomerOrderNumber='" + scanValue + "'");
            if (rowsLookup == null) return;
            if (rowsLookup.Count() > 0)
            {
                // Set status text
                this.lblStatusScan.Text = "OK";
                this.lblStatusScan.AppearanceItemCaption.ForeColor = Color.Green;

                // Found customer order
                for (int i = 0; i < rowsLookup.Count(); i++)
                {
                    int indexSelected = dataSource.Rows.IndexOf(rowsLookup[i]);
                    this.grvEDIOrderViewList.SetRowCellValue(indexSelected, "CheckDocument", true);

                    int ediID = Convert.ToInt32(this.grvEDIOrderViewList.GetRowCellValue(indexSelected, "EDI_OrderID"));
                    DataProcess<EDI_Orders> eidDA = new DataProcess<EDI_Orders>();
                    eidDA.ExecuteNoQuery(" Update EDI_Orders Set CheckDocument=1 Where EDI_OrderID={0}", ediID);
                }

                this.grvEDIOrderViewList.RefreshData();
            }
            else
            {
                // Set status text
                this.lblStatusScan.Text = "Not Found";
                this.lblStatusScan.AppearanceItemCaption.ForeColor = Color.Red;
            }

            this.mmScanbarcode.SelectAll();
            this.mmScanbarcode.Text = string.Empty;
            this.mmScanbarcode.Focus();
        }
        ...
        private void grvEDIOrderViewList_RowCellStyle(object sender, RowCellStyleEventArgs e)
        {
            if (e.RowHandle < 0) return;
            if (!e.Column.FieldName.Equals("CheckDocument")) return;
            bool isDocumentCheck = Convert.ToBoolean(e.CellValue);
            if (isDocumentCheck)
            {
        ...
        private void rpe_hle_QuantityOrders_Click(object sender, EventArgs e)
        {
            ...
            string dateFrom = Convert.ToDateTime(dateEditFromDate.EditValue).Date.ToString("yyyy-MM-dd");
            string dateTo = Convert.ToDateTime(dateEditToDate.EditValue).Date.ToString("yyyy-MM-dd");
            string ProductNumber = Convert.ToString(this.grvEDIOrderDetailsView.GetFocusedRowCellValue("ProductNumber"));
            var ExpDateN = Convert.ToString(this.grvEDIOrderDetailsView.GetFocusedRowCellValue("ExpiryDate"));
            var CustomerRefN = Convert.ToString(this.grvEDIOrderDetailsView.GetFocusedRowCellValue("CustomerRef"));
            if (ExpDateN != "")
            {
        ...
        private void rpe_hle_ViewAllRelatedOrders_Click(object sender, EventArgs e)
        {
        ...
        private void radioGroup_DispatchType_SelectedIndexChanged(object sender, EventArgs e)
        {
            RadioGroup v_rg = (RadioGroup)sender;
            int v_index = v_rg.SelectedIndex;
            if (v_index > 1 && v_index != 5)
                this.btn_ProcessSelected.Enabled = false;
            else
                this.btn_ProcessSelected.Enabled = true;
        }

        private void grvEDIOrderDetailsView_FocusedRowChanged_1(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            //if (this.grvEDIOrderDetailsView.FocusedRowHandle < 0) return;
            //if (this.ediDetailsSource == null) return;
            //string ReportDate = dateEditFromDate.DateTime.Date.ToString("yyyy-MM-dd");
            //String SQL = "STEDI_CheckStock '" + this.grvEDIOrderDetailsView.GetFocusedRowCellValue("ProductNumber") + "'," + this.lke_Customer.EditValue;
            //MessageBox.Show("STEDI_CheckStock '" + this.grvEDIOrderDetailsView.GetFocusedRowCellValue("ProductNumber") + "'," + this.lke_Customer.EditValue);
            //this.stockSource = FileProcess.LoadTable("STEDI_CheckStock '" + this.grvEDIOrderDetailsView.GetFocusedRowCellValue("ProductNumber") + "'," + this.lke_Customer.EditValue);
            //this.grcStockOnHand.DataSource = this.stockSource;
            //productID = Convert.ToInt32(this.grvEDIOrderDetailsView.GetFocusedRowCellValue("ProductID"));
        }

        private void btn_ProcessRef_Click(object sender, EventArgs e)
        {
            if (this.lke_Customer.EditValue == null) return;
            if (this.grvEDIOrderViewList.SelectedRowsCount <= 0)
            {
                    ...
                    if (v_EDIOrderType.Equals("DO"))
                    {
                        ...
                        if (v_ListCusClient.Count <= 0)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    XtraMessageBox.Show("==> Error is unexpected");
                                    //XtraMessageBox.Show("Error is unexpected message=[{0}],\n InnerException=[{1}],\n StackTrace=[{2}]", ex.Message, ex.InnerException, ex.StackTrace);
                                }
                        ...
                        DA.Warehouse.EDIOrdersDA v_DaTheLongLife = new EDIOrdersDA();
                        int v_Ret = v_DaTheLongLife.STEDI_ProcessDispatchingOrder_Main(v_EDIOrderID, AppSetting.CurrentUser.LoginName, 10);
                        //DataProcess<object> dataProcess = new DataProcess<object>();
                        //dataProcess.ExecuteNoQuery("STEDI_ProcessDispatchingOrder_Reference_NotMain " + v_EDIOrderID + ",'" + AppSetting.CurrentUser.LoginName + "'");
        ...
        private void btn_ProcessRefNew_Click(object sender, EventArgs e)
        {
            DataProcess<EDI_Orders> v_Da = new DataProcess<EDI_Orders>();
            DateTime v_fromDate = v_Da.GetDate().AddDays(-7);
            DateTime v_toDate = v_Da.GetDate().AddDays(+7);
            ...
            foreach (var index in this.grvEDIOrderViewList.GetSelectedRows())
            {
                ...
                if (v_EDIOrderType.Equals("DO"))
                {
                    ...
                    DataProcess<object> dataProcess = new DataProcess<object>();
                    int resultProcess = dataProcess.ExecuteNoQuery("STEDI_ProcessDispatchingOrder_Reference_NotMain " + v_EDIOrderID + ",'" + AppSetting.CurrentUser.LoginName + "'");
                    if (resultProcess < 0)
                    {
        ...
        private void grvOrderReview_CustomRowFilter(object sender, DevExpress.XtraGrid.Views.Base.RowFilterEventArgs e)
        {
            //if (showOnlySelected)
            //{
            //    e.Visible = selectedRows.Contains(e.ListSourceRow);
            //    e.Handled = true;
            //}
        }

        private void grvEDIOrderViewList_CustomRowFilter(object sender, DevExpress.XtraGrid.Views.Base.RowFilterEventArgs e)
        {
        ...
        {
            if (XtraMessageBox.Show("Process DO by new Rule: 1/2 sl + 14 days?", "WMS", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
            {
                return;
            }
            DataProcess<EDI_Orders> v_Da = new DataProcess<EDI_Orders>();
        ...
        private void btn_ProcessRefOnlyAccountAllStatus_Click(object sender, EventArgs e)
        {
            //Check if the User isSupervisor
            bool isSupervisor = UserPermission.CheckSupervisorByDepartment(AppSetting.CurrentUser.LoginName, 4);
            if (!isSupervisor)
            {
                XtraMessageBox.Show("Only Supevisor has authority to process all statuses", "TPWMS",
            ...
            foreach (var index in this.grvEDIOrderViewList.GetSelectedRows())
            {
                ...
                if (v_EDIOrderType.Equals("DO"))
                {
                    ...
                    DataProcess<object> dataProcess = new DataProcess<object>();
                    int resultProcess = dataProcess.ExecuteNoQuery("STEDI_ProcessDispatchingOrder_Reference_NotMain " + v_EDIOrderID + ",'" + AppSetting.CurrentUser.LoginName + "',1");
                    if (resultProcess < 0)
                    {
                        MessageBox.Show("Process Fail", "TPWMS", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
        ...
        private void btn_ProcessReferenceAllStatus_Click(object sender, EventArgs e)
        {
            //Check if the User isSupervisor
            bool isSupervisor = UserPermission.CheckSupervisorByDepartment(AppSetting.CurrentUser.LoginName, 4);
            if (!isSupervisor)
            ...
            foreach (var index in this.grvEDIOrderViewList.GetSelectedRows())
            {
                try
                {
                    ...
                    if (v_EDIOrderType.Equals("DO"))
                    {
                        ...
                        DA.Warehouse.EDIOrdersDA v_DaTheLongLife = new EDIOrdersDA();
                        int v_Ret = v_DaTheLongLife.STEDI_ProcessDispatchingOrder_Main(v_EDIOrderID, AppSetting.CurrentUser.LoginName, 101);
                        //DataProcess<object> dataProcess = new DataProcess<object>();
                        //dataProcess.ExecuteNoQuery("STEDI_ProcessDispatchingOrder_Reference_NotMain " + v_EDIOrderID + ",'" + AppSetting.CurrentUser.LoginName + "'");
        ...
        private void btn_ProcessByClient_Click(object sender, EventArgs e)
        {
            var dl = MessageBox.Show("Do you want to process combine slog, shelfLife and by client!", "TPWMS", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
            if (dl.Equals(DialogResult.No)) return;
            ...
            foreach (var index in this.grvEDIOrderViewList.GetSelectedRows())
            {
                ...
                if (v_EDIOrderType.Equals("DO"))
                {
                    ...
                    resultProcess = dataProcess.ExecuteNoQuery("STEDI_ProcessDispatchingOrder_Main_ByClientCombineSlog " + v_EDIOrderID + ",'" + AppSetting.CurrentUser.LoginName + "',1");
        ...
        private void btnClearProcess_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (!selectOrderNumber.Contains("DO-")) return;


            var dl = MessageBox.Show("Are you sure to UNPROCESS SELECTED DISPATCHING ORDERS ?", "TPWMS", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
            if (dl.Equals(DialogResult.No)) return;

            DataProcess<object> dataProcess = new DataProcess<object>();
            int resultProcess;

            string sql = "STEDI_UnProcessDispatchingOrder '" + selectOrderNumber + "', '" + AppSetting.CurrentUser.LoginName + "' ";
        ...
        {
            var dl = MessageBox.Show("Do you want to process all extent rule!", "TPWMS", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
            if (dl.Equals(DialogResult.No)) return;
            ...
            foreach (var index in this.grvEDIOrderViewList.GetSelectedRows())
            {
                ...
                {
                    XtraMessageBox.Show("Module Is Active DO!!!", "WMS", MessageBoxButtons.OK, MessageBoxIcon.Information);
